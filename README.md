...
8. **多账号风控隔离**：每账号独立持久化浏览器 profile + 一号一代理（sticky）+ 活跃账号错峰 + 扫描抖动，降低多号关联特征（见[配置](#配置)）。
   ...
   - **指纹自洽**：账号 UA 的 Chrome 版本自动对齐真实内核，`Sec-CH-UA` / `navigator.userAgentData` / `platform` / WebGL 三方一致；canvas / Audio 加同账号固定微噪声。
...
> 小红书「发布」为 API 直连（`app/platforms/xhs/`，`static/*.js` 是版本绑定的签名脚本，平台改版失效时更新它）；抖音 / 快手「发布」走浏览器自动化（`app/platforms/<平台>/publish.py`，选择器随平台改版可能失效，集中在文件顶部常量便于修）。
